  ...
  const handleRunCode = useCallback(async (code, explicitStdin) => {
    ...
    try {
      ...
      if (mountedRef.current) {
        const isOffline = err.message.includes('fetch') || err.message.includes('Failed to fetch') || err.message.includes('Network');
        const errorMsg = isOffline
          ? `Backend server not reachable. Note: Python, JavaScript, TypeScript, and SQLite execute directly in the browser! For ${state.language}, a connected backend server is required.`
          : `Execution error: ${err.message}`;
        setOutput({ type: 'error', content: '', error: errorMsg, status: isOffline ? 'Backend Offline' : 'Error' });
      }
    } finally {
