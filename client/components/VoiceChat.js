 ...
 * VoiceChat Component
 * 
 * Optional WebRTC-based voice chat.
 * Uses Socket.io for signaling (offer/answer/ICE candidates).
 * Peer-to-peer audio streams between users in the room.
 */
...
const VoiceChat = memo(function VoiceChat({ socket, currentUser }) {
  ...
  // Socket events for voice signaling
  useEffect(() => {
    if (!socket) return;

    socket.on('voice:peers', (peers) => {
      // When we join, we get the list of existing voice users
      setVoiceUsers(peers.map(p => ({ userId: p.userId, username: p.username })));
      // Initiate call to each existing peer
    ...
    socket.on('voice:offer', async (data) => {
      ...
      try {
        ...
        pc.onicecandidate = (e) => {
          if (e.candidate) socket.emit('voice:ice-candidate', { to: data.from, candidate: e.candidate });
        };
        pc.ontrack = (e) => {
          playRemoteAudio(e.streams[0], data.from);
        };
  ...
  async function createOffer(targetSocketId, username) {
    ...
    try {
      ...
      pc.onicecandidate = (e) => {
        if (e.candidate) socket.emit('voice:ice-candidate', { to: targetSocketId, candidate: e.candidate });
      };
      pc.ontrack = (e) => {
  ...
  return (
    <div className="px-3 py-2 bg-[#252526] border-b border-editor-border flex-shrink-0">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <svg className="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 11a7 7 0 01-7 7m0 0a7 7 0 01-7-7m7 7v4m0 0H8m4 0h4m-4-8a3 3 0 01-3-3V5a3 3 0 116 0v6a3 3 0 01-3 3z" />
          </svg>
          <span className="text-xs font-medium text-gray-400">Voice</span>
          {isInVoice && <span className="text-[10px] px-1.5 py-0.5 bg-green-600/20 text-green-400 rounded-full">{voiceUsers.length + 1} in call</span>}
        </div>

        <div className="flex items-center gap-1">
          {isInVoice && (
            <button onClick={toggleMute}
              className={`p-1.5 rounded-md transition-colors ${isMuted ? 'bg-red-600/20 text-red-400' : 'bg-[#2a2d2e] text-gray-300 hover:text-white'}`}
          ...
          <button onClick={isInVoice ? leaveVoice : joinVoice}
            className={`text-[10px] px-3 py-1.5 rounded-md font-medium transition-all ${isInVoice
      ...
      {isInVoice && voiceUsers.length > 0 && (
        <div className="flex flex-wrap gap-1 mt-2">
          {voiceUsers.map(u => (
            <span key={u.userId} className="text-[10px] px-2 py-0.5 bg-green-600/10 text-green-400 rounded-full border border-green-600/20">
              {u.username}
            </span>
          ))}
        </div>
