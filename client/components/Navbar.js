 ...
 * 
 * Includes: room code (simple 6-char), language selector,
 * connection status, file save/open buttons, panel toggles.
 */
  ...
  return (
    <nav className="flex items-center justify-between px-3 py-1.5 bg-[#252526] border-b border-editor-border z-50">
      <div className="flex items-center gap-2">
        <button onClick={() => router.push('/')} className="flex items-center gap-2 px-2 py-1 rounded-md hover:bg-[#2a2d2e] transition" title="Home">
          <div className="w-6 h-6 rounded-md bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-[10px] font-bold text-white">{'<>'}</div>
        ...
        {/* Room Code — simple 6-char */}
        <button onClick={handleCopy} className="flex items-center gap-1.5 px-2 py-1 rounded-md hover:bg-[#2a2d2e] transition group" title="Copy room code">
          <span className="text-xs text-gray-400 font-mono tracking-widest font-bold">{roomId}</span>
          {copied ? (
        ...
        <div className="relative">
          <button onClick={() => setLangOpen(!langOpen)} className="flex items-center gap-1.5 px-2.5 py-1 rounded-md hover:bg-[#2a2d2e] border border-editor-border/50 transition">
            <span className="text-xs font-mono font-bold px-1 rounded" style={{ color: currentLang.color }}>{currentLang.icon}</span>
          ...
          {langOpen && (
            <>
              ...
              <div className="absolute top-full left-0 mt-1 w-48 bg-[#252526] border border-editor-border rounded-lg shadow-xl z-50 py-1">
                {LANGUAGES.map(lang => (
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="w-px h-5 bg-editor-border" />
        <button onClick={onToggleOutput} className={`p-1.5 rounded-md transition ${outputOpen ? 'bg-[#2a2d2e] text-blue-400' : 'text-gray-500 hover:text-gray-300 hover:bg-[#2a2d2e]'}`} title="Toggle Output">
        ...
        <button onClick={onToggleChat} className={`p-1.5 rounded-md transition ${chatOpen ? 'bg-[#2a2d2e] text-blue-400' : 'text-gray-500 hover:text-gray-300 hover:bg-[#2a2d2e]'}`} title="Toggle Chat">
        ...
        <div className="flex items-center gap-1 px-2 py-1 bg-[#2a2d2e] rounded-md">
          <svg className="w-3.5 h-3.5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" /></svg>
