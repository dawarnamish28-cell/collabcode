/**
 * Navbar v9.0 — Fixed language dropdown + polished
 * 
 * The dropdown now uses a portal-style fixed position 
...
const Navbar = memo(function Navbar({
  ...
  useEffect(() => {
    if (!langOpen) return;
    const close = (e) => {
      if (!e.target.closest('.lang-dropdown-portal') && !e.target.closest('.lang-trigger-btn')) {
        setLangOpen(false);
      }
    };
  ...
  return (
    <>
      <nav className="flex items-center justify-between px-2 sm:px-3 h-9 bg-[#19191c] border-b border-[#282828] z-40 flex-shrink-0">
        {/* Left side */}
        <div className="flex items-center gap-1 sm:gap-1.5 min-w-0">
          ...
          {/* Language Selector — trigger only, dropdown is a portal */}
          <button
            ref={langBtnRef}
            onClick={toggleLangDropdown}
            className="lang-trigger-btn flex items-center gap-1 px-1.5 py-1 rounded-md hover:bg-[#222] transition flex-shrink-0"
          >
      ...
      {/* Language Dropdown Portal — rendered outside the nav to avoid overflow clipping */}
      {langOpen && (
        <div
          className="lang-dropdown-portal fixed z-[9999]"
          style={{ top: dropdownPos.top, left: dropdownPos.left }}
        >
          <div className="w-48 bg-[#1a1b1e] border border-[#333] rounded-xl shadow-2xl py-1 max-h-80 overflow-y-auto animate-fade-in"
