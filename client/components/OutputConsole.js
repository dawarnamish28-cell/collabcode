/**
 * OutputConsole Component — Production Grade
 * 
 * Rich terminal output display with:
 * - Separate stdout / stderr sections with color coding
 * - Execution metadata (engine, time, memory, exit code, language version)
 * - Expandable stdin input before running
 * - Output history (keeps last N runs)
 * - Copy-to-clipboard on output
 * - ANSI-like color rendering
 */
...
const OutputConsole = memo(function OutputConsole({ output, onClear, isRunning, onRunWithStdin, language }) {
  ...
  // Add to history when new output arrives
  useEffect(() => {
    if (output && output.content && output.type !== 'info') {
      setHistory(prev => [...prev.slice(-9), { ...output, timestamp: new Date() }]);
    }
  }, [output]);
    ...
    <div className="h-full flex flex-col bg-[#1a1b26]">
      ...
      <div className="flex items-center justify-between px-3 py-1.5 bg-[#252526] border-b border-editor-border flex-shrink-0">
        <div className="flex items-center gap-2">
          ...
          {/* Engine + timing badge */}
          {output?.engine && (
            <span className="text-[10px] px-1.5 py-0.5 rounded bg-[#2a2d2e] text-gray-500 font-mono ml-1">
              {output.engine}
            </span>
          )}
          {output?.executionTime && output.executionTime !== 'N/A' && (
        ...
        <div className="flex items-center gap-1">
          ...
          {output?.exitCode !== undefined && output?.exitCode !== null && output.exitCode !== 0 && (
            <span className="text-[10px] px-1.5 py-0.5 rounded bg-red-600/15 text-red-400 font-mono">
          ...
          <button
            ...
            className={`p-1 rounded transition-colors ${showStdin ? 'bg-blue-600/20 text-blue-400' : 'text-gray-500 hover:text-gray-300 hover:bg-[#2a2d2e]'}`}
          ...
          <button
            ...
            className="p-1 rounded text-gray-500 hover:text-gray-300 hover:bg-[#2a2d2e] transition-colors disabled:opacity-30"
            title="Copy output"
          >
            {copied ? (
      ...
      {showStdin && (
        <div className="px-3 py-2 bg-[#1e1f2e] border-b border-editor-border flex-shrink-0">
          <div className="flex items-start gap-2">
            <span className="text-[10px] text-gray-500 mt-1.5 font-mono select-none">stdin:</span>
            <textarea
              value={stdinValue}
              onChange={(e) => setStdinValue(e.target.value)}
              placeholder="Standard input (optional)..."
              rows={2}
              className="flex-1 bg-[#0d0e17] border border-editor-border/50 rounded px-2 py-1 text-xs
                       text-gray-300 font-mono placeholder-gray-600 resize-none
          ...
          {onRunWithStdin && (
            <div className="flex justify-end mt-1.5">
              <button
                onClick={() => onRunWithStdin(stdinValue)}
                disabled={isRunning}
                className="text-[10px] px-3 py-1 bg-green-600 hover:bg-green-500 text-white rounded
        ...
        {!output && !isRunning && (
          <div className="h-full flex items-center justify-center text-gray-600 text-xs">
            <div className="text-center space-y-2">
              ...
              <p className="text-gray-700 text-[10px]">Supports JavaScript, Python, C, and C++ with local execution</p>
            </div>
          </div>
        )}
        ...
        {isRunning && (!output || output.type === 'info') && (
          <div className="flex items-center gap-3 text-yellow-400 py-2">
            ...
            <div>
              ...
              <span className="text-xs text-yellow-600 block">Compiling and running in sandbox</span>
            </div>
          </div>
        )}
          ...
          <div className="space-y-2">
            ...
            {output.content && (
              <div>
                ...
                <pre className="whitespace-pre-wrap break-words text-gray-200 bg-[#12131e] rounded-md px-3 py-2 border border-green-500/10 leading-relaxed">
                  {output.content}
                </pre>
              </div>
            )}
            ...
            {output.error && (
              <div>
                ...
                <pre className="whitespace-pre-wrap break-words text-red-300 bg-red-950/30 rounded-md px-3 py-2 border border-red-500/15 leading-relaxed">
                  {output.error}
                </pre>
              </div>
            )}
            ...
            <div className="flex items-center gap-3 pt-1 text-[10px] text-gray-600 border-t border-gray-800 mt-2">
              {output.engine && (
                <span className="flex items-center gap-1">
                  ...
                  <span className="text-gray-400">{output.engine}</span>
                </span>
              )}
              {output.language && (
                <span className="flex items-center gap-1">
              ...
              {output.executionTime && output.executionTime !== 'N/A' && (
                <span className="flex items-center gap-1">
                  ...
                  <span className="text-gray-400">{output.executionTime}</span>
                </span>
              )}
              {output.memoryUsed && output.memoryUsed !== 'N/A' && (
                <span className="flex items-center gap-1">
                  <span className="text-gray-500">Memory:</span>
                  <span className="text-gray-400">{output.memoryUsed}</span>
                </span>
              )}
            </div>
          </div>
